...
if __name__ == '__main__':
  ...
  # 2017 Common Configuration
    config.General.workArea      = 'Zmuon_DataJobs_UL2017BCDEFG_DoubleMu_7Oct2021_10LSPerJob'
...
#    config.JobType.sendExternalFolder = True #I don't have an CMSSW_BASE/external so I don't think I need this 
    ...
    config.Data.lumiMask        = '/afs/cern.ch/cms/CAF/CMSCOMM/COMM_DQM/certification/Collisions17/13TeV/Legacy_2017/Cert_294927-306462_13TeV_UL2017_Collisions17_GoldenJSON.txt' #41.48 fb^-1, see: https://twiki.cern.ch/twiki/bin/viewauth/CMS/DCUserPage#Legacy_2017_aka_UL2017_datasets
...
#    config.Data.totalUnits      = 100 #for testing purposes 
    ...
    # Run2017B DoubleMu
    config.General.requestName = 'DoubleMuUL_Run2017B_7Oct2021_10LSPerJob' 
    config.Data.inputDataset   = '/DoubleMuon/Run2017B-09Aug2019_UL2017-v1/MINIAOD' 
    config.Data.outLFNDirBase  = '/store/user/mhadley/Zmuon_DataJobs_DiMu_UL2017B_7Oct2021_10LSPerJob'
    p = Process(target=submit, args=(config,))
    p.start()
    p.join()
    
  # Run2017C DoubleMu
    config.General.requestName = 'DoubleMuUL_Run2017C_7Oct2021_10LSPerJob' 
    config.Data.inputDataset   = '/DoubleMuon/Run2017C-09Aug2019_UL2017-v1/MINIAOD'
    config.Data.outLFNDirBase  =  '/store/user/mhadley/Zmuon_DataJobs_DiMu_UL2017C_7Oct2021_10LSPerJob'
    p = Process(target=submit, args=(config,))
    p.start()
    p.join()

  # Run2017D DoubleMu
    config.General.requestName = 'DoubleMuUL_Run2017D_7Oct2021_10LSPerJob' 
    config.Data.inputDataset   = '/DoubleMuon/Run2017D-09Aug2019_UL2017-v1/MINIAOD' 
    config.Data.outLFNDirBase  = '/store/user/mhadley/Zmuon_DataJobs_DiMu_UL2017D_7Oct2021_10LSPerJob' 
    p = Process(target=submit, args=(config,))
    p.start()
    p.join()
    
#   Run2017E DoubleMu    
    config.General.requestName = 'DoubleMuUL_Run2017E_7Oct2021_10LSPerJob' 
    config.Data.inputDataset   = '/DoubleMuon/Run2017E-09Aug2019_UL2017-v1/MINIAOD'
    config.Data.outLFNDirBase  =  '/store/user/mhadley/Zmuon_DataJobs_DiMu_UL2017E_7Oct2021_10LSPerJob' 
    p = Process(target=submit, args=(config,))
    p.start()
    p.join()
    
  #  Run2017F DoubleMu    
    config.General.requestName = 'DoubleMuUL_Run2017F_7Oct2021_10LSPerJob' 
    config.Data.inputDataset   = '/DoubleMuon/Run2017F-09Aug2019_UL2017-v1/MINIAOD'
    config.Data.outLFNDirBase  =  '/store/user/mhadley/Zmuon_DataJobs_DiMu_UL2017F_7Oct2021_10LSPerJob' 
    p = Process(target=submit, args=(config,))
    p.start()
    p.join()
...
#Was a 5 TeV run, Do NOT use (if you try to submit it to Crab, the lumi mask saves you, as crab finds no events from Era G in the Golden JSON, so you just get a SUBMITFAILED message)
  #  Run2017G DoubleMu    
#     config.General.requestName = 'DoubleMuUL_Run2017G_7Oct2021_10LSPerJob' 
#     config.Data.inputDataset   = '/DoubleMuon/Run2017G-09Aug2019_UL2017-v1/MINIAOD'
#     config.Data.outLFNDirBase  =  '/store/user/mhadley/Zmuon_DataJobs_DiMu_UL2017G_7Oct2021_10LSPerJob' 
#     p = Process(target=submit, args=(config,))
#     p.start()
#     p.join()
